...
"""Port of Freqtrade ``berlinguyinca/BbandRsi``."""
...
class BbandRsiConfig(FreqtradePortConfig, frozen=True):
    """Configuration for ``BbandRsi`` (ported from Freqtrade)."""

    bb_period: PositiveInt = 20
    bb_std: PositiveFloat = 2.0
    rsi_period: PositiveInt = 14
    rsi_buy_threshold: PositiveFloat = 30.0
    rsi_sell_threshold: PositiveFloat = 70.0


class BbandRsi(FreqtradeLongOnlyStrategy):
    """
    Bollinger Band + RSI mean reversion (long only).

  Ported from ``user_data/strategies/berlinguyinca/BbandRsi.py``.
    """

    def __init__(self, config: BbandRsiConfig) -> None:
    ...
    def check_entry(self, bar: Bar) -> bool:
        ...
        return (
            self._rsi.value < rsi_from_freqtrade(self.config.rsi_buy_threshold)
            and close < self._bb.lower
        )

    def check_exit(self, bar: Bar) -> bool:
        return self._rsi.value > rsi_from_freqtrade(self.config.rsi_sell_threshold)
