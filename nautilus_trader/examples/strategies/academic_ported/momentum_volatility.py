...
"""High-volatility momentum combination."""
...
class MomentumVolatilityConfig(FreqtradePortConfig, frozen=True):
    """Configuration for ``MomentumVolatility``."""

    momentum_days: PositiveInt = 126
    vol_period: PositiveInt = 126
    vol_sma_period: PositiveInt = 63
    historical_bars_days: PositiveInt = 250


class MomentumVolatility(FreqtradeLongOnlyStrategy):
    """
    Long when 6-month momentum is positive and volatility is elevated.

    Simplified port of
    ``momentum-and-reversal-combined-with-volatility-effect-in-stocks.py``.
    """

    def __init__(self, config: MomentumVolatilityConfig) -> None:
        super().__init__(config)
        self._momentum = LaggedReturn(config.momentum_days, 0)
    ...
    def _register_indicators(self) -> None:
        ...
        self.register_indicator_for_bars(bt, self._momentum)
    ...
    def check_entry(self, bar: Bar) -> bool:
        if not self._vol.initialized or not self._vol_sma.initialized:
            return False
        return self._momentum.value > 0 and self._vol.value > self._vol_sma.value

    def check_exit(self, bar: Bar) -> bool:
        if not self._vol.initialized or not self._vol_sma.initialized:
            return False
        return self._momentum.value <= 0 or self._vol.value <= self._vol_sma.value
